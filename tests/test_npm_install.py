...
def test_npm_install_uses_project_prefix(tmp_path: Path) -> None:
    ...
    with patch("shipgate.installer.subprocess.run") as run:
    ...
    assert prefix == tmp_path / ".shipgate" / "tools" / "npm"
    assert "jscpd" in argv
    assert "-g" not in argv
    assert (tmp_path / ".shipgate" / "tools" / "npm" / "package.json").exists()


def test_resolver_finds_managed_npm_binary(tmp_path: Path) -> None:
    bin_dir = tmp_path / ".shipgate" / "tools" / "npm" / "node_modules" / ".bin"
