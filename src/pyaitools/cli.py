"""pyaitools command-line interface."""
...
from pyaitools.gates import lib_path, scaffold_gate
from pyaitools.installer import Installer
from pyaitools.models import FailureReport
...
def gate_init(
    ...
    """Scaffold a script gate under .pyaitools/gates/ and register it in the catalog."""
    ...
    typer.echo("")
    typer.echo("Add the check to pyaitools.yaml:")
    typer.echo(f"  - id: {catalog_path.stem}")
    typer.echo("")
    typer.echo("Run it:")
    typer.echo(f"  pyaitools run --check {catalog_path.stem} --target .")
...
def run(
    ...
    suite_id = suite or (project_config.suite if project_config else None)
    if not suite_id:
        typer.echo("SETUP provide --suite or create pyaitools.yaml", err=True)
        raise typer.Exit(code=2)
