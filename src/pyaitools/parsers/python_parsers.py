...
def parse_ruff_json(stdout: str) -> list[Finding]:
    ...
    return [_ruff_json_finding(item) for item in payload]


def _ruff_json_finding(item: dict) -> Finding:
...
def parse_ruff_format(stdout: str, stderr: str) -> list[Finding]:
    text = strip_ansi(stdout or stderr)
    if not text.strip():
        return []
    findings, _current_file = _scan_ruff_format_lines(text.splitlines())
    if findings:
        return findings
...
def _scan_ruff_format_lines(lines: list[str]) -> tuple[list[Finding], str | None]:
    findings: list[Finding] = []
    current_file: str | None = None
    for line in lines:
        current_file = _ruff_format_file_from_line(line, current_file)
        finding = _ruff_format_finding_from_line(line, current_file)
        if finding is not None:
            findings.append(finding)
            continue
        _append_ruff_format_diff(findings, line, current_file)
    return findings, current_file


def _append_ruff_format_diff(findings: list[Finding], line: str, current_file: str | None) -> None:
...
def _ruff_format_file_from_line(line: str, current_file: str | None) -> str | None:
...
def _ruff_format_finding_from_line(line: str, current_file: str | None) -> Finding | None:
