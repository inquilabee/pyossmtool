...
from shipgate.models import CheckDef, IgnoreSpec, ProjectConfig, SuiteCheckRef, SuiteDef, ToolDef

BUNDLED_IGNORE_PATHS: tuple[str, ...] = (
    f"{RUNTIME_DIR}/",
    ".pyossmtool/",
    "venv/",
    ".venv/",
...
def resolve_effective_ignores(
    ...
    *,
    ...
    merged = merge_ignore_specs(
        IgnoreSpec(ignore_profile=[".gitignore"], ignore_paths=[*BUNDLED_IGNORE_PATHS, *(bundled_patterns or [])]),
...
def _keep_finding(finding, ignores: EffectiveIgnores) -> bool:
    from shipgate.models import Finding
...
def materialize_for_tool(
    ...
    *,
    ...
    from shipgate.ignore_materialize import materialize_for_tool as _materialize
...
def ignore_env(ignores: EffectiveIgnores, project_root: Path) -> dict[str, str]:
    ...
    env: dict[str, str] = {}
    if profiles:
        env["SHIPGATE_IGNORE_PROFILES"] = "\n".join(profiles)
    if ignores.path_patterns:
        env["SHIPGATE_IGNORE_PATHS"] = "\n".join(ignores.path_patterns)
    return env
...
def _normalize_pattern(pattern: str) -> str:
    ...
    normalized = normalize_finding_path(pattern) or ""
...
def _normalize_repo_path(path: str) -> str:
    ...
    return normalize_finding_path(path) or ""
...
def ignores_from_env(project_root: Path | None = None) -> EffectiveIgnores:
    """Build ignores from SHIPGATE_IGNORE_* environment variables (script gates)."""
    import os

    root = (project_root or Path.cwd()).resolve()
    profile_files = _resolve_env_profiles(root, os.environ.get("SHIPGATE_IGNORE_PROFILES", ""))
    path_patterns = _env_path_patterns(os.environ.get("SHIPGATE_IGNORE_PATHS", ""))
    return EffectiveIgnores(profile_files=profile_files, path_patterns=path_patterns)
