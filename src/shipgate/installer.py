...
from shipgate.models import EnvMode, InstallMethod, SuiteDef, ToolDef
...
class Installer:
    def __init__(
        ...
        self.managed_venv = self.tools_root / RUNTIME_DIR / "tools" / "venv"
    ...
    def _managed_python(self) -> str:
        ...
        shipgate itself supports 3.11–3.14. Some third-party CLI tools still lag
    ...
    def _install_npm(self, tool: ToolDef) -> None:
        ...
        npm_root = self.tools_root / RUNTIME_DIR / "tools" / "npm"
        npm_root.mkdir(parents=True, exist_ok=True)
        package_json = npm_root / "package.json"
        if not package_json.exists():
            package_json.write_text('{"name":"shipgate-tools","private":true}\n', encoding="utf-8")
        subprocess.run(
