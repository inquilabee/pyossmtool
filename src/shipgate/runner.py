...
from shipgate.constants import RUNTIME_DIR
from shipgate.discovery import argv_targets_for_check
from shipgate.gates import default_report_path
...
class Runner:
    def __init__(
        ...
        self.resolver = BinaryResolver(self.project_root, tools_root=self.tools_root)
    ...
    def run_suite(
        ...
        results = self._run_suite_checks(suite_id, suite, check_refs, env_mode, project_config, fail_fast, overrides)
        return SuiteResult(
            suite_id=suite_id,
    ...
    def _run_suite_checks(
        ...
        for check_ref in check_refs:
            target = (
                overrides.target
                if overrides and overrides.target
                else self._resolve_target(check_ref, suite, project_config)
            )
    ...
    def run_check(
        ...
        *,
        ...
        check_id = self.registry.resolve_check_id(check_id)
        ...
        if argv_targets is None:
            argv_targets = self._argv_targets(check_id, target, effective_ignores, overrides)
            if argv_targets is None:
    ...
    def _prepare_check_launch(
        ...
        *,
        ...
        cli_config = overrides.config_path if overrides else None
    ...
    def _build_native_argv(
        ...
        *,
        ...
        base_config_path = self.config_resolver.resolve_config_path(tool, project_config, cli_config=cli_config)
        ...
        if env_mode == EnvMode.PROJECT:
            argv, env = self._rebind_to_project_python(tool, binary, argv, env)
        elif tool.id == "mutmut" and self.resolver.resolve_env_mode(env_mode) == EnvMode.PROJECT:
            argv, env = self._rebind_to_project_python(tool, binary, argv, env)
        if tool.id == "mutmut":
            env = self._with_project_virtualenv(env)
        return argv, env

    def _with_project_virtualenv(self, env: dict[str, str]) -> dict[str, str]:
        for name in (".venv", "venv"):
            venv = self.project_root / name
    ...
    def _rebind_to_project_python(
        ...
        """Run PROJECT-mode tools with the project interpreter.

        Mutmut (and similar) may only be installed in the managed tools env. Invoking that
        console script uses managed Python (missing project deps). Prefer project ``python -m``.
        """
        if self._is_under_project_env(resolved_binary):
            return argv, self._with_project_import_path(env)
        project_python = self._project_python()
        if project_python is None:
            return argv, self._with_project_import_path(env)
        env = self._with_managed_site_packages(env)
        env = self._with_project_import_path(env)
        return [str(project_python), "-m", tool.id, *argv[1:]], env

    def _project_python(self) -> Path | None:
        for bin_dir in self.resolver.project_env_bin_dirs():
    ...
    def _is_under_project_env(self, binary: Path) -> bool:
        resolved = binary.resolve()
        for bin_dir in self.resolver.project_env_bin_dirs():
            try:
                resolved.relative_to(bin_dir.resolve())
                return True
            except ValueError:
                continue
        return False

    def _with_managed_site_packages(self, env: dict[str, str]) -> dict[str, str]:
        sites = sorted((self.tools_root / RUNTIME_DIR / "tools" / "venv" / "lib").glob("python*/site-packages"))
        if not sites:
            return env
        site = str(sites[-1])
        existing = env.get("PYTHONPATH", "")
        parts = [site, *[part for part in existing.split(os.pathsep) if part and part != site]]
        env["PYTHONPATH"] = os.pathsep.join(parts)
        return env

    def _with_project_import_path(self, env: dict[str, str]) -> dict[str, str]:
        """Prefer analysis-tree sources so worktree checkouts import the branch under test."""
        src = self.project_root / "src"
        extra = str(src if src.is_dir() else self.project_root)
        existing = env.get("PYTHONPATH", "")
        parts = [extra, *[part for part in existing.split(os.pathsep) if part and part != extra]]
        env["PYTHONPATH"] = os.pathsep.join(parts)
        return env
    ...
    def _native_config_argv(
        ...
        *,
        ...
        config_argv = self.config_resolver.extra_argv(tool, project_config, cli_config=cli_config)
    ...
    def _finalize_check_result(
        ...
        *,
        ...
        blocking = [finding for finding in findings if finding.severity == Severity.ERROR]
        if exit_ok and not blocking:
            return CheckResult(check_id=check_id, passed=True)
        findings = self._blocking_findings(blocking, exit_ok, completed, stdout)
    ...
    def _blocking_findings(
        ...
        if blocking:
            return blocking
        if not exit_ok:
            return [_exit_code_finding(completed.returncode, completed.stderr, stdout)]
        return []
    ...
    def _match_check_ref(self, refs: list[SuiteCheckRef] | None, check_id: str) -> SuiteCheckRef | None:
        if not refs:
            return None
        for ref in refs:
            if ref.id == check_id or self.registry.resolve_check_id(ref.id) == check_id:
                return ref
        return None
    ...
    def _argv_targets(
        ...
        check = self.registry.get_check(check_id)
        tool = self.registry.get_tool(check.tool)
        include_override = overrides.include_for(check.include) if overrides else None
        if overrides and overrides.include_globs:
            include_override = overrides.include_globs
