...
Shell-based policy checks integrated with shipgate failure reports.
...
config:
  ...
  project_file: .shipgate/configs/gates/gate.module-size.yaml
...
```yaml
# .shipgate/configs/gates/gate.module-size.yaml
scan_roots:
  - src/myapp
  - scripts
...
Or set explicit paths in `shipgate.yaml`:

```yaml
configs:
  mode: paths
  paths:
    gate.module-size: .shipgate/configs/gates/gate.module-size.yaml
```
...
```bash
shipgate gate init module-size --description "Cap Python module line counts"
# edit .shipgate/gates/module-size.sh
# add to shipgate.yaml checks: - id: gate.module-size
shipgate run --check gate.module-size --target src/
```
...
1. Source `defaults/gates/lib.sh` via `shipgate gates lib-path`
...
| Variable | Meaning |
| --------------------------- | ------------------------------------------------------------ |
| `SHIPGATE_ROOT` | Repository root |
| `SHIPGATE_TARGET` | Resolved scan target for this check |
| `SHIPGATE_CHECK_ID` | Catalog check id |
| `SHIPGATE_REPORT` | Path to write structured JSON findings |
| `SHIPGATE_IGNORE_PROFILES` | Newline-separated ignore profile files (`.gitignore` syntax) |
| `SHIPGATE_IGNORE_PATHS` | Newline-separated repo-relative paths/globs to skip |

Use `gate_path_ignored "<repo-relative-path>"` in bash gates to skip ignored files.

### Structured output

`gate_fail` / `gate_warn` append to `SHIPGATE_REPORT`:
...
On failure, shipgate normalizes this into `FailureReport` v1 under `reports/failures/`.
...
script: .shipgate/gates/module-size.sh
parser: gate_json
output_file: .shipgate/reports/gate.module-size.json
argv: []
success:
  exit_codes: [0]
```
