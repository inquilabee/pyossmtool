...
_gate_append_finding() {
	...
	local file="${4:-}"
	local line="${5:-}"
	python3 - "${SHIPGATE_REPORT}" "${rule_id}" "${severity}" "${message}" "${file}" "${line}" <<'PY'
import json
import sys
...
gate_path_ignored() {
	local rel_path="${1#./}"
	"${SHIPGATE_PYTHON:-python3}" - "${rel_path}" <<'PY'
import fnmatch
import os
import sys
from pathlib import Path
...
patterns: list[str] = [
    p.strip() for p in os.environ.get("SHIPGATE_IGNORE_PATHS", "").splitlines() if p.strip()
]
