...
from shipgate.models import CheckDef, ConfigMode, GateConfigSpec, ProjectConfig
...
def resolve_gate_config_path(
    ...
    *,
    ...
    for candidate in _convention_gate_config_paths(project_root, check.id):
        if candidate.exists():
...
def _convention_gate_config_paths(project_root: Path, check_id: str) -> list[Path]:
    return [
        project_root / f"{check_id}.yaml",
        project_root / PROJECT_CONFIGS_DIR / f"{check_id}.yaml",
    ]
...
def load_gate_config(
    ...
    *,
    ...
    path = resolve_gate_config_path(check, project_root, project_config, cli_config=cli_config)
