...
class ErrorFormatterKind(StrEnum):
    JQ = "jq"
    FINDING_LINE = "finding_line"


def _require_formatter_field(applies: bool, value: str | None, kind_name: str, field_name: str) -> None:
    if applies and not value:
        raise ValueError(f"error formatter kind '{kind_name}' requires '{field_name}'")


class ErrorFormatterSpec(BaseModel):
    """Custom error formatter declared in shipgate.yaml."""

    kind: ErrorFormatterKind
    program: str | None = None
    template: str | None = None

    @model_validator(mode="after")
    def validate_payload(self) -> ErrorFormatterSpec:
        _require_formatter_field(self.kind == ErrorFormatterKind.JQ, self.program, "jq", "program")
        _require_formatter_field(
            self.kind == ErrorFormatterKind.FINDING_LINE,
            self.template,
            "finding_line",
            "template",
        )
...
class ProjectConfig(BaseModel):
    ...
    ignore_profile: list[str] = Field(default_factory=list, alias="ignore-profile")
    ignore_paths: list[str] = Field(default_factory=list, alias="ignore-paths")
    error_format: str = Field(default="json", alias="error-format")
    error_formatters: dict[str, ErrorFormatterSpec] = Field(
        default_factory=dict,
        alias="error-formatters",
    )
