...
from shipgate.ignore import EffectiveIgnores, ToolIgnoreMaterial, _all_patterns, _merge_yaml_config
from shipgate.models import ExtendFrom, IgnoreKind, ToolDef, ToolIgnoreSpec

_TY_INCOMPATIBLE_CHARS = frozenset("$[]!{,")


def filter_patterns_for_tool(tool_id: str, patterns: list[str]) -> list[str]:
    """Drop ignore patterns a tool cannot consume (ty rejects gitignore-only syntax)."""
    if tool_id != "ty":
        return patterns
    return [pattern for pattern in patterns if not _is_ty_incompatible(pattern)]


def _is_ty_incompatible(pattern: str) -> bool:
    if pattern.startswith("!"):
        return True
    if any(char in pattern for char in _TY_INCOMPATIBLE_CHARS):
        return True
    return _has_incompatible_globstar(pattern)


def _has_incompatible_globstar(pattern: str) -> bool:
    if "**" not in pattern:
        return False
    if pattern == "**" or pattern.startswith("**/") or "/**" in pattern:
        return False
    return True
...
def materialize_for_tool(
    ...
    *,
    ...
    patterns = filter_patterns_for_tool(tool.id, _all_patterns(ignores))
    if not patterns or tool.ignore is None:
        return ToolIgnoreMaterial()

    cache_dir = project_root / PROJECT_CACHE_DIR / "ignores" / check_id
