...
from shipgate.models import CheckDef, ProjectConfig, SuiteCheckRef, SuiteDef
...
def format_check_argv(check_argv: list[str], argv_targets: list[str], format_values: dict[str, str]) -> list[str]:
    ...
    return drop_empty_flag_pairs(formatted)


def drop_empty_flag_pairs(argv: list[str], flag: str = "--config") -> list[str]:
    """Remove a flag when its value is empty (optional layered --config sources)."""
    cleaned: list[str] = []
    index = 0
    while index < len(argv):
        ...
        if part != flag:
            ...
            index += 1
            continue
        pair, advance = _flag_pair(argv, index, flag)
        cleaned.extend(pair)
        index += advance
    return cleaned


def _flag_pair(argv: list[str], index: int, flag: str) -> tuple[list[str], int]:
    has_value = index + 1 < len(argv)
    next_value = argv[index + 1] if has_value else ""
    if next_value and not next_value.startswith("-"):
        return [flag, next_value], 2
    return [], 2 if has_value else 1
