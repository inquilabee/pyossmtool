...
from shipgate.parsers.gates import CliTextParser
from shipgate.parsers.patterns import FallbackTextParser, LineRegexParser
...
class MutmutParser(FallbackTextParser):
    id = "mutmut_text"
    _spinner_re = re.compile(r"^[\s\u2800-\u28ff⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏]*Generating mutants\s*$", re.IGNORECASE)
    _stats_re = re.compile(r"^[\s\u2800-\u28ff⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏]*running stats\s*$", re.IGNORECASE)
    _collect_error_re = re.compile(r"^_{5,}\s*ERROR collecting\s+(?P<file>\S+)\s*_{5,}\s*$")

    def parse(self, stdout: str, stderr: str = "", *, check: CheckDef | None = None) -> list[Finding]:
        text = (stdout or stderr).strip()
        if not text or self._all_passed(text):
            return []
        findings = self._structured_findings(text)
        if findings:
            return findings
        summary = self._summary_message(text)
        return [Finding(rule_id="mutmut", severity=Severity.ERROR, message=summary)]

    def _all_passed(self, text: str) -> bool:
        lowered = text.lower()
        return "passed" in lowered and "failed" not in lowered and "error" not in lowered

    def _structured_findings(self, text: str) -> list[Finding]:
        findings: list[Finding] = []
        for line in text.splitlines():
            match = self._collect_error_re.match(line.strip())
            if match:
                findings.append(
                    Finding(
                        rule_id="collect-error",
                        severity=Severity.ERROR,
                        message=f"ERROR collecting {match.group('file')}",
                        location=Location(file=match.group("file")),
                    )
                )
                continue
            if "ModuleNotFoundError:" in line or "ImportError:" in line:
                findings.append(Finding(rule_id="mutmut", severity=Severity.ERROR, message=line.strip()))
        return findings

    def _summary_message(self, text: str) -> str:
        useful = [line.strip() for line in text.splitlines() if line.strip() and not self._is_noise(line.strip())]
        if useful:
            return "\n".join(useful[-12:])
        return "mutmut failed (see raw tool output)"

    def _is_noise(self, line: str) -> bool:
        if self._spinner_re.match(line) or self._stats_re.match(line):
            return True
        return self._is_divider_noise(line)

    @staticmethod
    def _is_divider_noise(line: str) -> bool:
        if line.startswith("=") and len(line) > 10:
            return True
        return line.startswith("_") and "ERROR collecting" not in line
