...
from pyossmtool.gate_config import gate_env_from_config, load_gate_config
...
from pyossmtool.models import CheckDef, SuiteCheckRef, SuiteDef
...
def script_gate_env(
    ...
    *,
    ...
    env.update(
        {
            "PYOSSMTOOL_ROOT": str(runner.project_root),
            "PYOSSMTOOL_TARGET": target,
            "PYOSSMTOOL_CHECK_ID": check_id,
            "PYOSSMTOOL_PYTHON": sys.executable,
        }
    )
    if report_rel:
        env["PYOSSMTOOL_REPORT"] = str(runner.project_root / report_rel)
    _config_path, gate_config = load_gate_config(check, runner.project_root, project_config)
    resolved_config = runner.project_root / ".pyossmtool" / "cache" / f"{check_id}.config.yaml"
    resolved_config.parent.mkdir(parents=True, exist_ok=True)
    resolved_config.write_text(yaml.safe_dump(gate_config, sort_keys=False), encoding="utf-8")
    env["PYOSSMTOOL_GATE_CONFIG"] = str(resolved_config)
